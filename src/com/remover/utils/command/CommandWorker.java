package com.remover.utils.command;
...
public class CommandWorker implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, org.bukkit.command.Command cmd, String lbl, String[] args) {
		if(cmd.getName().equals(commandName))
		{
			if(args.length == 0)
			{
				sender.sendMessage(messages.getString("Help.AllowedCommands"));
			...
			else
			{
				for(Command c : commands)
				{
					if(c.getNames().contains(args[0].toLowerCase()))
					{
						if(sender instanceof Player)
						{
							...
							else sender.sendMessage(messages.getString("Error.WithoutPerms") + ": " + cmd.getName() + " " + args[0]);
							return true;
						}
				...
				sender.sendMessage(messages.getString("Error.NotFound") + ": " + cmd.getName() + " " + args[0]);
			}
			return true;
		}
		return false;
